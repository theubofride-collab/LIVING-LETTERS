...
export default function AdminCategoriesPage() {
  ...
  const handleDelete = async (cat: Categorie) => {
    ...
    await categorieService.delete(cat.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modal === 'add') {
        const created = await categorieService.create({ nom: form.nom, description: form.description })
        setCategories(prev => [...prev, created])
      } else if (modal === 'edit' && selected) {
        const updated = await categorieService.update(selected.id, { nom: form.nom, description: form.description })
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-serif text-2xl lg:text-3xl font-bold text-brand-dark">Catégories</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-brand-cream-dark text-center py-16">
          <p className="text-brand-muted">Aucune catégorie. Créez-en une !</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-brand-cream-dark">
                {categories.map(cat => (
                  <tr key={cat.id} className="hover:bg-brand-cream/30 transition-colors">
                    <td className="px-4 py-3">
                      <p className="font-semibold text-brand-dark">{cat.nom}</p>
                    </td>
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 animate-slide-up">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input required className="input-field" placeholder="Nom de la catégorie" value={form.nom}
                  onChange={e => setForm(f => ({ ...f, nom: e.target.value }))} />
              </div>
              <div>
                <label className="label">Description</label>
                <textarea rows={3} className="input-field resize-none" placeholder="Description…" value={form.description}
